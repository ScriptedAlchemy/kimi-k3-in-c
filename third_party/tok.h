/* tok.h - byte-level BPE tokenizer in C (cl100k / o200k / tiktoken families).
 *
 * A faithful reimplementation of HuggingFace tokenizer.json semantics:
 *   - model.type = BPE, ignore_merges = true, byte_fallback = false
 *   - pre_tokenizer: regex Split (cl100k or o200k pattern) + ByteLevel(add_prefix_space=false)
 *   - merge rank = position in the merges list; \p{L}/\p{N}/\s from tok_unicode.h
 *   - added_tokens (special or not) are atomic in both encode and decode
 *
 * API:
 ...
 *
 * This project never calls tok_load: Kimi K3 ships no tokenizer.json. The structures
 * below are populated directly from the released tiktoken.model by
 * src/tokenizer/k3_tok.h, after which tok_encode/tok_decode/tok_id_of work unchanged.
 * See NOTICE for the modifications made to this file.
 */
#ifndef TOK_H
#define TOK_H
#define _GNU_SOURCE
...
/* ---------- hash map (binary keys carrying an explicit length) ---------- */
...
typedef struct {
    ...
    char **id2str; int *id_added; int n_ids;   /* id -> string; id_added=1 for an added token (emitted literally) */
    int *id_special;                            /* 1 = added token with "special":true. These are CONTROL
                                                 * tokens (<|user|>, <|assistant|>, <sop>, ...) and are
                                                 * never legitimate response content. Distinct from
                                                 * id_added, which also covers <think>/<tool_call>
                                                 * ("special": false) -- those ARE real text and must
                                                 * be rendered. */
...
static int u8_next(const unsigned char *s, int len, int i, uint32_t *cp){
    ...
    *cp=c; return 1;   /* invalid byte: treated as a single unit */
}
...
static void tok_load(Tok *T, const char *path){
    ...
    /* Highest id, used to size id2str. The ids come from a tokenizer.json that may have
     * been fetched from an untrusted mirror: a NEGATIVE id would index id2str[id] BELOW
     * the allocation (an out-of-bounds write), and an added_token missing "id" or
     * "content" would dereference NULL. Both are checked rather than assumed. */
    int maxid=0;
    ...
    /* vocab: string -> id  (power-of-two capacity, ~2-3x load) */
    ...
    /* added tokens, special or not: atomic, emitted literally */
    if(added){
    ...
    /* arena/buf stay allocated: the j_dup strings are independent mallocs and must outlive them */
    (void)arena;
}

/* ---------- BPE over one piece: raw bytes [a,b) -> ids appended to out ---------- */
...
/* ---------- pre-tokenizer regex (cl100k pattern) over a span of text ----------
 * Decodes the codepoints, applies the alternatives IN ORDER, and calls bpe_piece on each
 * resulting piece. The order is part of the specification, not an optimisation: an
 * earlier alternative that matches wins even where a later one would match more. */
static void pretok_chunk(Tok *T, const unsigned char *p, int a, int b, int *out, int *no, int max){
    ...
    while(i<n){
        ...
        i++;  /* backstop: should be unreachable   */
...
/* ---------- pre-tokenizer o200k (GPT-4o regex family) ----------
 * Split regex:
...
/* ---------- decode: ids -> text (inverse byte-level map; added tokens literal) ---------- */
